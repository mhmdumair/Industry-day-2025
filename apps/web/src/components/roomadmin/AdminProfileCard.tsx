...
function ProfileCardContent() {
  ...
  useEffect(() => {
    setLoading(true);
    api
      // Changed to expect a single AdminProfile object, not an array
      .get<AdminProfile>("/room-admin/by-user")
      .then((res) => {
        ...
        if (adminProfile) {
          ...
          setError("No room admin data found.");
        }
        setLoading(false);
      })
      .catch((err) => {
  ...
  const handleSave = async () => {
    ...
    try {
      await api.patch(`/room-admin/${profileData.roomAdminID}`, updatePayload);
