...
import VacancyViewerDialog from "../../components/home/vacancy-viewer-dialog";
import {
    Dialog,
    DialogContent,
    DialogHeader,
...
// Transformed sponsor data shape
...
export default function AnnouncementsPage() {
    ...
    // State for sponsor details dialog
    const [dialogOpen, setDialogOpen] = useState(false);
    const [selectedSponsor, setSelectedSponsor] = useState<TransformedSponsor | null>(null);

    // State for vacancy viewer dialog
    const [vacancyDialogOpen, setVacancyDialogOpen] = useState(false);
    const [companyForVacancies, setCompanyForVacancies] = useState<{ id: string, name: string } | null>(null);
    ...
    const transformSponsorData = (sponsor: Sponsor): TransformedSponsor => ({
        companyID: sponsor.companyID, // Pass companyID
    ...
    const handleViewVacancies = (companyId: string, companyName: string) => {
        setCompanyForVacancies({ id: companyId, name: companyName });
        setVacancyDialogOpen(true);
        setDialogOpen(false); // Close the details dialog
    };

    const handleSponsorClick = (sponsor: Sponsor) => {
        setSelectedSponsor(transformSponsorData(sponsor));
        setDialogOpen(true);
    };
    ...
    return (
        <>
                    ...
                    <div className="space-y-6 sm:space-y-8">
                        {mainSponsor && (
                            <div className="flex justify-center">
                                <div onClick={() => handleSponsorClick(mainSponsor)} className="w-full aspect-[2/1] sm:aspect-[3/1] bg-white hover:bg-gray-100 dark:hover:bg-gray-900 transition-colors cursor-pointer flex items-center justify-center rounded-none sm:rounded-none border border-gray-200 dark:border-gray-700">
                                    {mainSponsor.logo ? (
                                        <img
                        ...
                        {silverSponsors.length > 0 && (
                            <div className={`grid gap-4 sm:gap-6 ${silverSponsors.length === 1 ? 'grid-cols-1 max-w-sm mx-auto' : silverSponsors.length === 2 ? 'grid-cols-2' : 'grid-cols-2 sm:grid-cols-3'}`}>
                                {silverSponsors.map((sponsor) => (
                                    <div key={sponsor.companyID} onClick={() => handleSponsorClick(sponsor)} className="aspect-square bg-white hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors cursor-pointer flex items-center justify-center rounded-none sm:rounded-none border border-gray-200 dark:border-gray-700">
                                        {sponsor.logo ? (
                                            <img
                                                src={`${sponsor.logo}`}
                        ...
                        <>
                            <DialogHeader>
                                <div className="flex items-center gap-4">
                                    <img src={selectedSponsor.logo} alt={`${selectedSponsor.name} logo`} className="h-16 w-16 object-contain rounded-none border p-1" />
                                    <div>
                                        <DialogTitle className="text-2xl dark:text-white">{selectedSponsor.name}</DialogTitle>
                                        <DialogDescription>{selectedSponsor.category} Sponsor</DialogDescription>
                                    </div>
                                </div>
                            </DialogHeader>
                            <div className="py-4 space-y-4 text-sm">
                                <p className="text-gray-600 dark:text-gray-300">{selectedSponsor.description}</p>
                                <div className="space-y-2">
                                    <p><strong>Contact:</strong> {selectedSponsor.contactPerson} ({selectedSponsor.designation})</p>
                                    <p><strong>Location:</strong> {selectedSponsor.location}</p>
                                </div>
                            </div>
                            <DialogFooter className="gap-2 sm:justify-between">
                                <Button
                                    onClick={() => handleViewVacancies(selectedSponsor.companyID, selectedSponsor.name)}
            ...
            {/* Vacancy Viewer Dialog */}
            {companyForVacancies && (
                <VacancyViewerDialog
                    companyId={companyForVacancies.id}
                    companyName={companyForVacancies.name}
