...
export default function CompanyFilter() {
  ...
  /* ----------------  init  ---------------- */
  useEffect(() => {
    const idFromUrl = new URLSearchParams(window.location.search).get('companyId');
    if (idFromUrl) setCompanyId(idFromUrl);
    else {
      setError('Company ID not found in URL'); setLoading(false);
    }
  }, []);
