...
const InterviewRegistration = () => {
    ...
    const load = async () => {
      try {
        const [cRes, iRes] = await Promise.all([
          ...
          axios.get(`/api/interview/student/${studentID}`)
        ]);
  ...
  const register = async (companyID: string) => {
    ...
    try {
      ...
      /* refresh interview list so buttons disable correctly */
      const iRes = await axios.get(`/api/interview/student/${studentID}`);
      setMyInterviews(iRes.data);
    } catch (e: any) {
