...
export class StudentService {
  ...
  async remove(id: string): Promise<{ message: string }> {
    try {
      // 1. Find Student and associated User ID
      ...
      // 2. Remove CV Files from Drive (if existing)
      const cv = await this.cvService.findByStudentId(id);
      if (cv && cv.cvID) {
          // The remove method in CvService handles file deletion from drive
          await this.cvService.remove(cv.cvID); 
      }

      // 3. Remove Student Record
      // (Cascading delete might handle CV record, but we ensured file deletion above)
      ...
      return { message: `Student ${id} and associated user removed successfully` };
    } catch (error) {
      if (error instanceof NotFoundException) {
        throw error;
      }
      throw new InternalServerErrorException('Failed to remove student and user');
    }
  }
}