...
export class CompanyService {
  ...
  async updateCompanyPassword(userId: string, dto: UpdatePasswordDto) {
  ...
  async adminResetCompanyPassword(companyId: string) {
    const company = await this.companyRepository.findOne({ where: { companyID: companyId } });
    if (!company) {
      throw new NotFoundException(`Company ${companyId} not found`);
    }
    return this.userService.resetPassword(company.userID);
  }
